...
<body>
	...
	<section id="cabecera">
		<div class="contenedor">
			<div id="Mensaje">
				<h1 style=" font-family: 'Yatra One'">Seleccionar Ciudad</h1>
				<p style=" font-family: 'Yatra One'">Como vemos en esta sección encontramos la lista de ciudades
					por lo tanto solo está el nombre.
				</p>
			</div>
		</div>
	</section>
	<section id="Boletin">
		<div class="contenedor">
			<form method="POST" action="select.php" method="post" enctype="multipart/form-data">
				<h1 class="animate__animated animate__backInLeft" style="color:  #360617; font-size:50px; font-family: 'Yatra One', cursive;text-align:center">Listado de Ciudades</h1>
				<center>
					<table style="font-family: 'Yatra One';font-size:30px">
						...
						while ($Fila = mysqli_fetch_assoc($ejecutar)) {
							$id = $Fila['Idciudad'];
							$no_u = $Fila['Nombreciudad'];
							$i++;
						?>
							<td><?php
								$sub_sql_1 = "SELECT Nombreciudad FROM ciudad WHERE Idciudad=" . $id;
				...
				if (isset($_GET['eliminar'])) {
					...
					$eliminar = "DELETE FROM ciudad WHERE Idciudad='$borrar_id'";
					$ejecutar = mysqli_query($conexion, $eliminar);

					if ($ejecutar) {
						echo "<script>alert('Esta ciudad ha sido eliminada!')</script>";
